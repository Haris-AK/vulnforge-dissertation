...
\maketitle{%
	...
	Practical cyber security education often relies on Capture the Flag exercises and cyber ranges to give learners hands on experience with vulnerabilities.  However, many existing platforms use static, pre authored challenges, which can reduce replayability once solutions are known or memorised.

	This project presents VulnForge, an educational platform that investigates whether generative AI can be used to create, deploy, verify, and clean up short-lived vulnerable Linux environments for cyber security training.

	VulnForge combines a web application, structured large language model generation, Ansible automation, Proxmox hosted Ubuntu LXC containers, and an
	AI supported tutoring interface. Learners can select a vulnerability category and difficulty level, launch an isolated environment, connect to it over SSH, attempt exploitation, submit a flag, and receive contextual guidance. Because model generated content cannot be assumed to be correct or safe, the system constrains generation through schemas, policy checks, deployment restrictions, health checks, flag protection checks, exploit verification, privilege separation, and automated cleanup.
	...
	Overall, VulnForge demonstrates that AI assisted generation of vulnerable training environments is technically viable in a controlled educational setting, but also highlights the importance of validation and containment. The project shows promise for improving challenge variety and replayability, while identifying reliability, safety, and pedagogical calibration as key areas for future work.
}
