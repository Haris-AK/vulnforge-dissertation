...
% =============================================================================
\documentclass[serif,oneside,numeric,raggedright,fullwidth]{qdissertation}
...
% Font
% Use Times New Roman with LuaLaTeX when it is installed. If it is not available
% on the build machine, fall back to TeX Gyre Termes, a Times-compatible TeX font.
% Do not load legacy pdfTeX font packages such as mathptmx here; they break
% Unicode/OpenType math output under LuaLaTeX.
% \IfFontExistsTF{Times New Roman}{%
%   \setmainfont{Times New Roman}[Ligatures=TeX]%
% }{%
%   \PackageWarningNoLine{main}{Times New Roman not found; using TeX Gyre Termes instead}%
%   \setmainfont{TeX Gyre Termes}[Ligatures=TeX]%
% }%
% \IfFontExistsTF{Arial}{%
%   \setsansfont{Arial}[Scale=MatchLowercase,Ligatures=TeX]%
% }{%
%   \setsansfont{TeX Gyre Heros}[Scale=MatchLowercase,Ligatures=TeX]%
% }%
% \IfFontExistsTF{Courier New}{%
%   \setmonofont{Courier New}[Scale=MatchLowercase,Ligatures=TeX]%
% }{%
%   \setmonofont{Source Code Pro}[Scale=MatchLowercase,Ligatures=TeX]%
% }%
% \setmathfont{texgyretermes-math.otf}[Scale=MatchLowercase]
% \setmonofont{Iosevka Nerd Font Mono}[Scale=MatchLowercase]
...
% then frontlists: TOC, List of Figures, List of Tables, List of Listings,
% List of Algorithms, List of Acronyms (if acronyms.tex defines \newacronym).
...
\maketitle{%
	...
	Practical cyber security education often relies on \gls{ctf} exercises and cyber ranges to give learners hands on experience with vulnerabilities.  However, many existing platforms use static, pre authored challenges, which can reduce replayability once solutions are known.

	This project presents VulnForge, an educational platform that investigates whether generative AI can be used to create, deploy, verify, and clean up vulnerable Linux environments for cyber security training.

	VulnForge combines a web application, structured \gls{llm} generation, Ansible automation, Proxmox hosted Ubuntu LXC containers, and an
	AI supported tutoring interface. Learners can select a vulnerability category and difficulty level, launch an isolated environment, connect to it over SSH, attempt exploitation, submit a flag, and receive contextual guidance. Because model generated content cannot be assumed to be correct or safe, the system constrains generation through schemas, health checks, exploit verification, and automated cleanup.

	The platform was evaluated using a full lifecycle harness across 75 generated challenges covering five vulnerability categories and three difficulty
	levels. The results showed that 57 out of 75 runs completed successfully end to end, with high generation, deployment, injection, and cleanup success rates. The main weaknesses appeared in verification and exploit reliability, showing that although the architecture is feasible, generated challenges do not always produce a consistent or reliable exploit path.

	Overall, VulnForge demonstrates that LLM generated vulnerable environments can work, but also highlights the importance of validation and containment. The successful runs demonstrate that the approach is feasible in a controlled prototype, but the failed runs show that generation alone is not enough, the environment and checks surrounding it are just as important. The main lesson from the project is that there is promise for improving challenge variety and replayability, while identifying reliability, safety, and pedagogical calibration as key areas for future work.
}
...
% parties to the work.
I thank my supervisor Amir Javed for his invaluable support and advice throughout this project.
...
Data supporting this dissertation is available on request at vulnforge@troto.net.

\section{Code}
% Software or code produced in this work and where it is available (repository, DOI, etc.).
Code produced for this dissertation is available on request at vulnforge@troto.net.
